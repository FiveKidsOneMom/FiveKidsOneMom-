      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-12 sm:mb-16">
          <div>
            ...
            <h2
              className="scroll-reveal font-display font-black text-brand-green-dark leading-tight"
          ...
          <Link
            href="/blog"
            className="scroll-reveal font-body text-sm font-semibold text-brand-green-mid hover:text-brand-green-dark underline underline-offset-4 transition-colors duration-200 self-start sm:self-auto"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          {featured.map((post) => (
            <article key={post.slug} className="scroll-reveal">
              <Link
                href={`/blog/${post.slug}`}
                className="blog-card block bg-brand-green-dark p-8 sm:p-10 h-full group focus-visible:outline-2 focus-visible:outline-brand-green-mid"
                aria-label={`Read: ${post.title}`}
              >
                <div className="flex items-center justify-between mb-6">
                  <span
                    className={`font-mono-brand text-xs tracking-widest uppercase px-2 py-1 ${categoryColors[post.category] ?? 'bg-brand-tan text-brand-green-dark'}`}
                  >
                    {post.category}
                  </span>
                ...
                <h3 className="font-display font-bold text-brand-cream text-xl sm:text-2xl leading-tight mb-4 group-hover:text-brand-green-mid transition-colors duration-200">
                  {post.title}
                </h3>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className="font-body text-brand-green-mid text-sm font-semibold group-hover:translate-x-1 transition-transform duration-200"
                    aria-hidden="true"
                  >
                    Read →
                  </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {regular.map((post) => (
            <article key={post.slug} className="scroll-reveal">
              <Link
                href={`/blog/${post.slug}`}
                className="blog-card block border border-brand-tan-light p-6 h-full group focus-visible:outline-2 focus-visible:outline-brand-green-mid"
                aria-label={`Read: ${post.title}`}
              >
                ...
                <h3 className="font-display font-bold text-brand-green-dark text-lg leading-tight mb-3 group-hover:text-brand-green-mid transition-colors duration-200">
                  {post.title}
                </h3>

                <p className="font-body text-brand-green-dark text-sm leading-relaxed mb-4">
                  {post.excerpt}
                </p>

                <time
                  dateTime={post.date}
                  className="font-mono-brand text-brand-green-dark/50 text-xs"
                >
                  {formatDate(post.date)}
                </time>
              </Link>
